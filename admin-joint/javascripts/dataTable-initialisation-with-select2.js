...
$(document).ready(function() {
    ...
    $(".datatable thead th").each(function(i){
        ...
        columnOptions += '<label><input type="checkbox" data-type="'+ $(this).data("type") +'" checked="checked" />'+ $(this).text() +'</label></li>';
    });
    ...
    var oTable = $('.datatable').DataTable( {
        "sDom": "<'t-header clearfix'<'t-actions btn-toolbar'><'t-controls'<'t-filters'>><'t-data-filter-wrapper'<'t-search'f><'t-data-filter'>>r><'t-alert'><'datatable-wrapper't><'t-footer clearfix'<'t-footer-info'<'t-length'l><'t-info'i>><'t-pages'p>>",
    ...
    // 加入功能列 & 繳費顯示切換
    //$(".t-actions").prepend('<div class="btn-group"><a href="#" class="btn btn-primary">群組寄信</a><a href="#" class="btn btn-primary">群組簡訊</a></div><div class="btn-group"><a href="#" class="btn btn-primary" data-toggle="modal" data-target="#tagsModal">標籤</a></div>');
    //$(".t-actions").prepend('<div class="btn-group"><a href="#" class="btn btn-primary"><i class="fa fa-envelope"></i> 寄信</a><a href="#" class="btn btn-primary" data-toggle="modal" data-target="#tagsModal"><i class="fa fa-tags"></i>標籤</a></div>');

    // 插入先前生成的顯示欄位選單內容 columnOptions
    //$(".t-filters").prepend('<div class="btn-group check-list"><a href="#" class="btn btn-default dropdown-toggle" data-toggle="dropdown">顯示欄位 <span class="caret"></span></a><div class="dropdown-menu"><ul role="menu" class="col-menu">'+columnOptions+'</ul><div class="dropdown-footer"><a href="#" class="btn btn-default btn-apply-display-setting">套用</a></div></div></div>');
    ...
    // 用選單切換欄位顯示
    $(document).on("click", ".check-list label", function(e){
        e.stopPropagation();
    });
    $(document).on("click", ".btn-apply-display-setting", function(e) {
        e.preventDefault();
        $(".col-menu label").each(function(i){
